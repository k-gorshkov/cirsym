...
char settings_file_name[20];

FILE *out,*inpa,*set;

char *b,*c,sp;
unsigned char powr[100];
unsigned long leng=0,lengl,lepr=0;
int flag_fi3,flag_fi4,flag_fi5,flag_fsn1,flag_fsn2,
    extract,flag_mat,
    fl_pow,fl_2m,fl_rg,flag_e,cirf,noequ,flag_pln,nuz=0,flag_sp,
	flag_cL,flag_num,flag_nul,flag_two,flag_thr,nnod=139,
	flag_out,flag_opt,st_pln,bi3=0,bi4=0,bi5=0,fl_reac,fl_tria,
	fl_ord,flag_2,flag_3,flag_4,flag_5,fl_g,fl_sp,fl_ran,ranmax,
	fl_loop,fl_mGc,fl_mGm;
float range2,range3,range4,range5,
      range1p,range2p,range3p,range4p,range5p;
...
/* "ЇаRй?-Ёп ЇаЁ  ┐Є-гвле ??-?а вRа е Ё<Ё ЇаЁ?┐-ЁЄ е "? */
...
void main(int argc, char* argv[])
{
 ...
 int nr,nc,nl,ng,nn,nm,nq,nt,ns,nk,nf,nh,
     nsub,da,ne,nj,nu,ni,dui,nej,fl;

 long double vl,fbeg;
 char st[30],nbr[20],el[100],str[80],str0[80],type,
      n1,n2,n3,n4,*t,t0[20];
 SOURCE *act;
 PASSIVE *matr, *diagui;
 struct time timen;
 ...
 if ((set=fopen(settings_file_name,"r"))!=NULL)
  {
   ...
   fgets(el,99,set);
   fgets(powr,99,set);
   fgets(el,99,set); /* задание режимов p-бисекции */
   fscanf(set,"%d %f\n",&flag_5,&range5p);
   fgets(el,99,set);
   fscanf(set,"%d %f\n",&flag_4,&range4p);
   fgets(el,99,set);
...
beg:
 ...
 for (m=0;; m++)
	...
	leng=0; lepr=0; nuz=0;
     ...
     diagui=(PASSIVE *) malloc (10*sizeof(PASSIVE));
 ...
 for (i0=0;i0<1000;i0++)
    {
     ...
     if (el[0]=='.' && (el[1]=='e' || el[1]=='E')) goto new;
     else
	...
	strcat(t,nbr);
	// str[k-1]='\0';
	if (flag_out) fprintf(out,"%s=%s;\n",str0,str);
//	if (flag_out) fprintf(out,"%s=%Lg;\n",str0,vl);
	actall(n1,n2,n3,n4,1,str0,&act_n,act);
       }
/*    if (type=='G' && el[1]!='c' && el[1]!='0')
       {
	str[0]=type;
	str[1]='\0'; t=str;
	strcat(t,nbr);
	...
	str[1]='\0'; t=str;
	strcat(t,nbr);
	if (flag_out) fprintf(out,"%s=p*%Lg;\n",str,vl);
	actall(n1,n2,n3,n4,1,str,&act_n,act);
       }    
 if (type=='G' && el[1]=='0')
       {
...
if (flag_nul && nm+nq+nt)
  {
   ...
   SOURCE *act1;
   act1 = (SOURCE *) malloc (20*sizeof(SOURCE));
 ...
 if (flag_out) 
{
 ...
 nr,ng,nc,nl,ns,nk,nf,nh,nn,nm,nq,nt);
	...
	if (fl_loop)
	  {paslp(&n,matr); actlp(&act_n,act,&fl); 
       if (fl==2) goto exi;}
	if (flag_mat == -1) detanm(act_n,act);
    else {	
          if (!dui || da)
	    {
