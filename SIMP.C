...
extern int flag_fi3,flag_fi4,flag_fi5,flag_fsn,extract,
           fl_tria,flag_e,cirf,noequ,flag_pln,nuz,
		   flag_cL,flag_nul,st_pln;
...
void reallyy(int *n,PASSIVE *a1)
{
 ...
 for (j=0;j<*n;j++)
    {
     if (   a1[j].reb[0]=='r' || a1[j].reb[0]=='R' 
	     || a1[j].reb[0]=='z' || a1[j].reb[0]=='Z' 
	     || a1[j].reb[0]=='L' || a1[j].reb[0]=='c' && flag_pln
 	    ) continue;
     for (i=0;i<*n;i++)
	{
	 if (i == j) continue;
	 if (   a1[i].reb[0]=='r' || a1[i].reb[0]=='R' 
	     || a1[i].reb[0]=='z' || a1[i].reb[0]=='Z' 
		 || a1[i].reb[0]=='L' || a1[i].reb[0]=='c' && flag_pln) 
		 continue;
 ...
 for (i=0;i<p;i++)
	...
	if (//   str[i]=='5'	||  str[i]=='1' || 
     ...
     for (j=0;j<*n;j++)
	    ...
	    if (a1[j].v1==str[i] || a1[j].v2==str[i]) k++;
	    }
    if (k==2)
       {
	send=0; rec=0;
	...
	t0=(char*) malloc(strlen(a1[red[0]].reb)+
	                  strlen(a1[red[1]].reb)+30);
	t0[0]='\0'; t=t0;
	y0=(char*) malloc(30);
	y0[0]='\0'; y=y0;
	strcat(y,"Ym"); y+=2;
	sprintf(y,"%d",++nuz);
 	strcat(t,y0); t+=strlen(y0);
	strcat(t++,"=");
if (a1[red[0]].kol > 1) strcat(t++,"(");
	strcat(t,a1[red[0]].reb); t+=strlen(a1[red[0]].reb);
if (a1[red[0]].kol > 1) strcat(t++,")");
	strcat(t++,"*");
if (a1[red[1]].kol > 1) strcat(t++,"(");
 ...
 void reallp(int *n,PASSIVE *a1)
{
 ...
 for (j=0;j<*n;j++)
    {
     if (a1[j].reb[0]!='c') continue;
     for (i=0;i<*n;i++)
	{
	 if (i == j) continue;
	 if (a1[i].reb[0]!='c') continue;
	 if (   a1[j].v1==a1[i].v1 && a1[j].v2==a1[i].v2
	     || a1[j].v1==a1[i].v2 && a1[j].v2==a1[i].v1)
	   {
	    char *t,*t0;

	    t0=(char*) malloc(strlen(a1[i].reb)+strlen(a1[j].reb)+100);
	    t0[0]='\0'; t=t0;
	    strcat(t,a1[i].reb); t+=strlen(a1[i].reb); free(a1[i].reb);
	    if (a1[j].reb[0] != '-') strcat(t++,"+");
	    strcat(t,a1[j].reb);
	 ...
	 if (j != i && act[j].kol < 0)
	   {
	    ...
	    if (s1==t1) {x1=s2; x2=t2; si=1; goto sear;}
	    else if (s1==t2) {x1=s2; x2=t1; si=2; goto sear;}
	    else if (s2==t1) {x1=s1; x2=t2; si=3; goto sear;}
	    else if (s2==t2) {x1=s1; x2=t1; si=4; goto sear;}
	   }
	 continue;
sear:
 	 for (m=0;m<*n;m++)
	    if (   (matr[m].reb[0] == 'g' || matr[m].reb[0] == 'y'
		 || matr[m].reb[0] == 'Y' || matr[m].reb[0] == 'c')
		 && (   matr[m].v1==x1 && matr[m].v2==x2
		     || matr[m].v1==x2 && matr[m].v2==x1))
	     frmatr(m,n,matr);
	 for (m=0;m<*act_n;m++)
	    {
	     if (m != i && m != j)
		   {
		    ...
		    if (t1==x1 && t2==x2 || t1==x2 && t2==x1)
			  {	
		       if (act[m].kol > 0 && act[m].reb[0] == 'G')
	             {act[m].kol=0; *fl=3; return;}
			   else if (act[m].kol < 0) {*fl=2; return;}
	          }
            if (fl_tria) {			  
			if (!pr) {t1=act[m].v3; t2=act[m].v4;}
		    else {t1=act[m].v1; t2=act[m].v2;}
		    if (t1==x1 && t2==x2 || t1==x2 && t2==x1)
		      { 
			  if (si==1 || si==2) {if (!pr) 
			     {act[i].v1=x2; act[i].v2=x1;}
				 else
			  {act[i].v3=x2; act[i].v4=x1;}}
              else		  
				 if (si==3 || si==4) {if (!pr) 
				 {act[i].v1=x1; act[i].v2=x2;}
			     else
			     {act[i].v3=x1; act[i].v4=x2;}}
			  } 
			}		  
		  } 
		 }
 ...
 for (i=0;i<p;i++)
    {
     ...
     if (kr=='0') continue;
     k=0;
	 for (j=0;j<n;j++) if (matr[j].v1==kr || matr[j].v2==kr) k++;
     for (j=0;j<*act_n;j++)
	if ((act[j].reb[0]=='M')
	    &&
	     (kr==act[j].v1 || kr==act[j].v2
